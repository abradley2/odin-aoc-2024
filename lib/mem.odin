...
check_leaks :: proc(tracking_allocator: ^mem.Tracking_Allocator) {
	if len(tracking_allocator.allocation_map) > 0 {
		for _, entry in tracking_allocator.allocation_map {
			missing_free_loc := fmt.aprintfln("leaked - %v bytes @ %v", entry.size, entry.location)
			fmt.printf(missing_free_loc)
	...
	if len(tracking_allocator.bad_free_array) > 0 {
		for entry in tracking_allocator.bad_free_array {
			bad_free_loc := fmt.aprintfln("double free - %p @ %v", entry.memory, entry.location)
			defer delete(bad_free_loc)
			fmt.printf(bad_free_loc)
